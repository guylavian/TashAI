...
// Build the fully-wired Fastify app WITHOUT listening or warming models, so tests
// can `inject()` against it. The bootstrap at the bottom (listen + warmModels)
...
export async function buildApp(): Promise<FastifyInstance> {
  ...
  // ─── Auth — the gateway is the only trusted upstream. When RELAY_API_KEY is set
  // it guards EVERY route except GET /health and GET /metrics (scrape/liveness).
  const apiKey = config.server.apiKey;
  if (apiKey) {
    const want = Buffer.from(apiKey);
    app.addHook("onRequest", async (req, reply) => {
      const path = req.url.split("?")[0];
      if (req.method === "GET" && (path === "/health" || path === "/metrics")) return;
...
async function warmModels(app: FastifyInstance): Promise<void> {
...
// Only bootstrap when run directly (`node dist/index.js` / `tsx src/index.ts`),
// not when imported by tests via buildApp(). "type":"commonjs" → require.main works.
