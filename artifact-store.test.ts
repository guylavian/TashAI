...
const T = "default"; // tenant scope (migration added a tenant first arg)
...
function roundtrip() {
  ...
  assert.strictEqual(get(T, "deadbeef"), undefined, "unknown hash → undefined");
}

function sliceWithQuery() {
  const lines = ["a0", "a1", "a2", "MATCH-here", "a4", "a5", "a6"];
  const h = put(T, lines.join("\n"), "log");
  const s = slice(T, h, "match")!; // case-insensitive
  ...
  assert.ok(slice(T, h, "nope")!.startsWith("no lines match"), "no match → notice");
  assert.strictEqual(slice(T, "0".repeat(40), "x"), undefined, "missing artifact → undefined");
  // no query → head, capped
  const big = put(T, "x".repeat(20000), "big");
  assert.strictEqual(slice(T, big, undefined, 8000)!.length, 8000, "no query → head capped at maxChars");
}

function evictionBound() {
  const hashes: string[] = [];
  for (let i = 0; i < 60; i++) hashes.push(put(T, `entry-${i}`, `f${i}`)); // 60 > cap 50
  assert.strictEqual(get(T, hashes[0]), undefined, "oldest evicted past the 50-entry bound");
  assert.ok(get(T, hashes[59]), "newest still present");
}

function ttlExpiry() {
  const h = put(T, "perishable", "t");
  assert.ok(get(T, h), "present before TTL");
  const realNow = Date.now;
  Date.now = () => realNow() + 61 * 60 * 1000; // jump >1h
  try {
    assert.strictEqual(get(T, h), undefined, "expired after TTL");
  } finally {
    Date.now = realNow;
  }
}
